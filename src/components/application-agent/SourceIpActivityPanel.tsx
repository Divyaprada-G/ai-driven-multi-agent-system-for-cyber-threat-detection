...
import { SourceIpAppActivityItem } from '../../types/application';

interface Props {
  sourceIps: SourceIpAppActivityItem[];
}

export const SourceIpActivityPanel: React.FC<Props> = ({ sourceIps }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [sortField, setSortField] = useState<'requestCount' | 'failedRequests' | 'suspiciousEvents'>('requestCount');

  const filteredIps = sourceIps
    .filter(ip => ip.sourceIp.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <div id="source-ip-activity-panel" className="space-y-3">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Server className="w-4 h-4 text-violet-500" />
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
            Source IP Client Behavioral Analytics ({sourceIps.length})
          </h3>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              ...
              else if (sortField === 'failedRequests') setSortField('suspiciousEvents');
          ...
          >
            ...
            Sort: {sortField === 'requestCount' ? 'Volume' : sortField === 'failedRequests' ? 'Failures' : 'Threats'}
          </button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 font-semibold">
              <tr>
                <th className="py-2.5 px-4">Client Source IP</th>
                <th className="py-2.5 px-3">Total Ingress</th>
                <th className="py-2.5 px-3">Distinct Endpoints</th>
                <th className="py-2.5 px-3">Failed Requests</th>
                <th className="py-2.5 px-3">Auth Failures</th>
                <th className="py-2.5 px-3">Security Threats</th>
              ...
              {filteredIps.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-6 text-center text-slate-400 font-sans">
                    No client IPs match filter.
                  </td>
                </tr>
              ) : (
                filteredIps.map(ip => (
                  <tr key={ip.sourceIp} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                    ...
                    <td className="py-2.5 px-3">{ip.uniqueEndpoints}</td>
                    <td className="py-2.5 px-3">
                      <span className={ip.failedRequests > 0 ? 'text-rose-600 dark:text-rose-400 font-semibold' : 'text-slate-400'}>
                        {ip.failedRequests}
                      </span>
                    </td>
                    <td className="py-2.5 px-3">
                      <span className={ip.authFailures > 0 ? 'text-amber-600 dark:text-amber-400 font-semibold' : 'text-slate-400'}>
                        {ip.authFailures}
                      </span>
                    </td>
                    <td className="py-2.5 px-3">
                      <span className={ip.suspiciousEvents > 0 ? 'text-rose-600 dark:text-rose-400 font-bold' : 'text-slate-400'}>
                        {ip.suspiciousEvents}
                      </span>
                    </td>
                    <td className="py-2.5 px-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${getRiskBadge(ip.riskIndicator)}`}>
                        {ip.riskIndicator}
                      </span>
                    </td>
                  </tr>
